    ...
    while True:
        ...
        if OneDirection == 'D' or  OneDirection == 'B'  :
            ...
            if not y in range(0,10) or  x not in range(0,10):
                print('Não é um valor valido') # TODO: trocar esses 10 por uma váriavel que é o range do tabuleiro
            else:
                
                
                if y + int(tipo) < 10 or  x + int(tipo) < 10 : return (int(x),int(y),OneDirection) # retorna o x , y e a direção válidos	# se a posição mais o tipo < 10, então é válido e sai do laço while
